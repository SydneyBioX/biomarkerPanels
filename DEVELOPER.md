...
### 2. Feature Pre-selection
- **Files:** `discriminative_features.R`, `ruv_features.R`, `transferable_features.R`, `denominator_features.R`, `cpop_fit.R`, `cpop_select.R`
- **Purpose:** Large omics datasets are computationally expensive to optimize directly. Pre-selection filters the feature space using various statistical methods (e.g., Wilcoxon, RUV, CPOP) to form a highly-qualified "feature pool" that is small enough for NSGA-II/III to efficiently search.
