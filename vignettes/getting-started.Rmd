...
By default candidates are scored with a (regularized) logistic model fitted by
cross-validation. A custom `scoring_fn` is only used on the in-sample scoring
path, so it requires `fitness_cv = FALSE` and `regularized = FALSE` — note that
this combination scores candidates on their own training data and is prone to
overfitting.
